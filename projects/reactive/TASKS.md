...
- `display/table` Simple Table cleanup follow-ups:
  - Replace the local `CheckField` stub in `src/components/display/table/table.tsx` with `@reformjs/reactive/fields/check-field`. Adapt `onRowSelect` / `onHeaderSelect` (and `handleHeadCheck`) to the new `onChange(checked, meta)` two-arg signature. Until this is done, the `selectable` column renders blank cells.
  - Once the CheckField wiring is fixed, port the `selectable` and `filterable` stories into `projects/reactive-storybook/src/library/display/table/stories/`. `filterable` also depends on a `Data` / `DataAccess` / `DataTextProperty` context, which doesn't currently exist in this project — confirm those are available (or stub equivalents) before porting.
...
- Rename old "scheme" naming convention to "interactive" convention. The following remnants need to be updated:
  ...
  - `sch-check-{color}` CSS class in `src/components/fields/check-field/check-field.tsx`
