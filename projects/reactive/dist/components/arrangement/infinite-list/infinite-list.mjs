import React, { forwardRef, useMemo, useRef, useState, useLayoutEffect, useCallback, useEffect, useImperativeHandle } from 'react';
import classnames from 'classnames';
import { isNil, isFinite, isNumber } from 'lodash-es';
import scrollIntoView from 'scroll-into-view-if-needed';
import '../../../common/prop-types.mjs';
import useThrottleCallback from '../../../hooks/use-throttle-callback.mjs';
...
function validateIndexes(minIndex, maxIndex, firstIndex, lastIndex, startingIndex) {
    let minIndexes = null;
    let maxIndexes = null;
...
const InfiniteList = forwardRef((props, ref) => {
    const { id, className, Component, children, minIndex, maxIndex, firstIndex, lastIndex, startingIndex, onLoad, loadCount, bufferCount, items, hold, keepLoaded, onTopIndexChange, topOffset, ...rest } = props;
    if (isNil(Component)) {
    ...
    const { minIndexes, maxIndexes, startingIndexes, multipliers, firstCount, startingCount, lastCount, } = useMemo(() => validateIndexes(minIndex, maxIndex, firstIndex, lastIndex, startingIndex), [minIndex, maxIndex,
        firstIndex,
    ...
    const review = useCallback(() => {
        ...
        const { count: beforeCount, } = getNumberChildrenBefore(listRef.current, itemsRef.current, topOffsetRef.current, !isNil(onTopIndexChange));
    ...
    const handleScroll = useThrottleCallback(review, [review], 200, { leading: false });
    ...
    return (React.createElement(Component, { ...rest, ref: listRef, id: id, className: classnames('ra-infinite-list', className), onScroll: handleScroll },
        React.createElement(InfiniteListContext.Provider, { value: data }, children)));
});
