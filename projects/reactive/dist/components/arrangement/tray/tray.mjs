...
import { noop, isNil, forEach, merge } from 'lodash-es';
...
function determineAllowedHeight(windowHeight, positionValues) {
    const { fromPosition, // Where to start the calc from.
    toPosition, } = positionValues;
...
function attemptLevelTop(renderingParameters) {
    ...
    const { anchorRect, windowHeight, sidePadding, offset: { top: topOffset }, trayRect, } = renderingParameters;
    ...
    const allowedMaxHeight = determineAllowedHeight(windowHeight, {
...
function attemptLevelBottom(renderingParameters) {
    const { anchorRect, windowHeight, sidePadding, offset: { bottom: bottomOffset }, trayRect, } = renderingParameters;
...
function attemptLevelMiddle(renderingParameters) {
    const { sidePadding, windowHeight, trayRect, anchorRect, } = renderingParameters;
    const finalHeight = trayRect.height;
    const styles = {};
...
function centerHorizontally(anchorRect, finalWidth, windowWidth, sidePadding) {
    let offsetLeft = 0;
    let offsetRight = 0;
...
function dockLeftRight(anchorRect, finalWidth, windowWidth, sidePadding, dockRight) {
...
function determineLeftAlignment(alignmentValues, force) {
    const { anchorRect, finalWidth, windowWidth, sidePadding, horizontallyCenter, dockRight, } = alignmentValues;
    ...
    if (horizontallyCenter) {
        return {
            styles: {
                left: buildPx(centerHorizontally(anchorRect, finalWidth, windowWidth, sidePadding)),
            },
            location: 'middle',
        };
    }
    return dockLeftRight(anchorRect, finalWidth, windowWidth, sidePadding, dockRight);
}
function getFinalHeights(renderingParameters, positionValues, isClipped, force) {
...
function attemptBottomRender(renderingParameters, force) {
    const { anchorRect, windowWidth, sidePadding, paddingValue, trayRect, horizontallyCenter, dockRight, tailOffset, } = renderingParameters;
    ...
    const finalHeights = getFinalHeights(renderingParameters, {
        fromPosition: top,
        toPosition: sidePadding,
    }, isClipped, force);
...
// If it does then render there.
function attemptTopRender(renderingParameters, force) {
    const { anchorRect, windowHeight, windowWidth, sidePadding, paddingValue, trayRect, horizontallyCenter, dockRight, tailOffset, } = renderingParameters;
...
// If it does then render there.
function attemptLeftRender(renderingParameters, force) {
    const { anchorRect, trayRect, paddingValue, windowWidth, sidePadding, tailOffset, windowHeight, } = renderingParameters;
    ...
    styles.right = buildPx(windowWidth - anchorRect.left + paddingValue + tailOffset);
...
// If it does then render there.
function attemptRightRender(renderingParameters, force) {
    const { anchorRect, sidePadding, paddingValue, windowWidth, trayRect, tailOffset, windowHeight, } = renderingParameters;
    ...
    if (anchorRect.right + trayRect.width + paddingValue + sidePadding > windowWidth) {
        // This will keep the tray from spilling outside the screen.
...
function executeRenderMethods(dropPositions, renderingParameters, force = false) {
    let results = null;
    ...
    forEach(dropPositions, (dropPosition) => {
        ...
        const trayRenderResult = attemptRenderMethod(renderingParameters, force);
...
function setTrayStyles(trayElement, dropPositions, renderingParameters) {
    let trayRenderResult = executeRenderMethods(dropPositions, renderingParameters);
    if (isNil(trayRenderResult)) {
        // If a good candidate was not found lets see if can
        // find one that at least works well enough.
        trayRenderResult = executeRenderMethods(dropPositions, renderingParameters, true);
    }
...
function setVerticalTailLocation(tailRenderingParameters) {
    const { location, anchorRect, trayRect, tailRect, windowHeight, } = tailRenderingParameters;
    const styles = {};
    if (location === 'top') {
        ...
        if (anchorRect.height < trayRect.height) {
            ...
            if (anchorRect.bottom > trayRect.bottom) {
                ...
                // dock the arrow to the bottom of the tray.
                styles.bottom = buildPx(windowHeight - trayRect.bottom + cornerRadius);
            }
            else {
                const anchorCenter = anchorRect.height / 2;
                const tailCenter = tailRect.height / 2;
...
function setTailStyles(tailElement, tailRenderingParameters) {
    const { position, location, windowWidth, anchorRect, trayRect, zIndex, tailRect, } = tailRenderingParameters;
    const styles = {
        zIndex,
    };
    if (position === 'bottom') {
        ...
        if (location === 'left') {
            ...
            styles.left = buildPx(anchorRect.left + (anchorCenter - tailCenter));
        }
    }
        ...
        merge(styles, setVerticalTailLocation(tailRenderingParameters));
    }
...
class Tray extends Component {
    ...
    updatePosition(isOpen, wasOpen) {
        ...
        const { getAnchor, dropPositions, parentPadding, offset, horizontallyCenter = false, dockRight = false, enableTail, } = this.props;
        if (!isOpen && !wasOpen) {
            return;
        }
        ...
        const { position, location } = setTrayStyles(trayElement, dropPositions || DEFAULT_DROP_POSITIONS, renderingParameters);
    ...
    render() {
        const { renderIndex } = this.context;
        const { id, className, children, onClick, open, enableTail, maxHeight, getAnchor, } = this.props;
        ...
        const tail = enableTail ? (React.createElement("div", { ref: this.tailRef, className: classnames('ra-tray-tail', {
                'active-hidden': !open,
            }) })) : null;
        const tray = (React.createElement(TrayContext.Provider
        ...
        , { 
            ...
            React.createElement("div", { id: id, role: "presentation", ref: this.trayRef, className: classnames('ra-tray', { 'active-hidden': !open }, className), tabIndex: -1, onClick: onClick, onFocus: this.handleFocus, onBlur: this.handleBlur, style: {
                    maxHeight,
                    minWidth,
                    maxWidth,
                } }, children),
