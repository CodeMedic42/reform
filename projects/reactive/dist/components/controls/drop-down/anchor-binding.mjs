...
class AnchorWrapper extends Component {
    constructor(props) {
        super(props);
    ...
    focus() {
        ...
        const focusTarget = getTarget(this.anchorRef.current, focusTargetSelector);
        if (!isNil(focusTarget)) {
            focusTarget.focus();
        }
    }
    render() {
        const { AnchorComponent, bindingInterface: { onClick, onFocus, onKeyPress, onKeyDown, onPointerEnter, onPointerLeave, onPointerCancel, }, anchorProps = {}, open, } = this.props;
        return (React.createElement("div", { ref: this.anchorRef, className: classnames('anchor-binding'), tabIndex: -1, onClick: onClick, onFocus: onFocus, onKeyPress: onKeyPress, onKeyDown: onKeyDown, onPointerEnter: onPointerEnter, onPointerLeave: onPointerLeave, onPointerCancel: onPointerCancel },
            React.createElement(AnchorComponent, { ...anchorProps, open: open })));
    }
}
 ...
 */
function applyAnchorBinding(AnchorComponent, options = {}) {
    const { focusSelector, boundingSelector, } = options;
    ...
    const AnchorBinding = (props) => {
        ...
        return (React.createElement(AnchorWrapper, { ...props, ref: bindingRef, AnchorComponent: AnchorComponent, boundingTargetSelector: boundingSelectorLookup(anchorProps), focusTargetSelector: focusSelectorLookup(anchorProps) }));
    };
...
export { AnchorWrapper, applyAnchorBinding as default };
