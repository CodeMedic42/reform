...
import { noop, debounce, isNil, get } from 'lodash-es';
import Tray from '../../arrangement/tray/tray.mjs';
import Provider from './drop-down-context.mjs';
...
class DropDown extends Component {
    constructor(props) {
        ...
        this.handleWindowEvent = debounce(this.handleWindowEvent.bind(this), 200, {
            leading: true,
            trailing: false,
        });
    ...
    handleButtonClick(event) {
        ...
        const { openOnClick = DEFAULTS.openOnClick, onAnchorClick, } = this.props;
        const { open } = this.state;
    ...
    setOpen(open) {
        const { disabled, onOpen, onClose, autoCloseTime, } = this.props;
        if (disabled) {
            return;
        }
    ...
    renderMainControl() {
        const { Anchor, anchorProps, openOnHover, onKeyDown, } = this.props;
        const { open } = this.state;
        if (isNil(Anchor)) {
            throw new Error('Need Anchor');
        }
        return (React.createElement(Anchor, { anchorProps: anchorProps, open: open, bindingRef: this.anchorRef, bindingInterface: {
    ...
    render() {
        const { id, className, trayClassName, children, dropPositions, minTrayWidth, maxTrayWidth, maxTrayHeight, horizontallyCenter, dockRight, enableTail, onClick, keepLoaded, } = this.props;
        const { open, openFromHover, openFromFocus } = this.state;
        let tray = null;
        if (open || keepLoaded) {
            tray = (React.createElement(Provider, { value: { open } }, children));
        }
        return (
        ...
        React.createElement("div", { ref: this.mainRef, id: id, className: classnames('ra-drop-down', className, {
                open,
            }), onKeyUp: this.handleKeyUp, onKeyPress: this.handleButtonKeyPress, onKeyDown: this.handleKeyDown, onBlur: this.handleBlur, onClick: onClick },
            this.renderMainControl(),
            React.createElement(Tray, { className: classnames('ra-drop-down-tray', trayClassName), ref: this.trayRef, open: open || openFromHover || openFromFocus, id: !isNil(id) && id.length > 0 ? `${id}-tray` : undefined, dropPositions: dropPositions, getAnchor: this.getAnchor, onClick: this.handleTrayClick, minWidth: minTrayWidth, maxWidth: maxTrayWidth, maxHeight: maxTrayHeight, horizontallyCenter: horizontallyCenter, dockRight: dockRight, enableTail: enableTail }, tray)));
    }
}
DropDown.propTypes = {
    id: PropTypes.string,
