import React, { forwardRef, useRef, useState, useCallback, useImperativeHandle, useMemo } from 'react';
import PropTypes from 'prop-types';
import classnames from 'classnames';
import { isNil, trim, findIndex, forEach, map, toLower, reduce, isString, get, isFunction, isNumber } from 'lodash-es';
import useCollectionContext from '../../../../hooks/use-collection-context/use-collection-context.mjs';
import DropDown from '../../../controls/drop-down/drop-down.mjs';
import '../../../controls/drop-down/anchors/anchor-button.mjs';
import DropDownList from '../../../controls/drop-down/drop-down-list.mjs';
import Spinner from '../../../display/spinner/spinner.mjs';
...
import SelectText from './select-text.mjs';
...
function getId(option, optionValuePath) {
    ...
    if (isNil(optionValuePath)) {
        throw new Error('optionValuePath is required when not using strings for values.');
    }
    let key = null;
...
function getOptionText(option, optionLabelPath) {
    ...
    if (!isString(option) && !isNumber(option)) {
        if (isNil(optionLabelPath)) {
            throw new Error('optionLabelPath is required when not using strings for options.');
        }
...
function buildFinalOptions(items, filterValue, customValueSetting, filterMatchesValue) {
...
function buildAnchorProps({ anchorProps, id, color, value, title, describedBy, labelledBy, disabled, size, placeholder, ariaLabel, listBoxId, }) {
    const props = {
        ...anchorProps,
        id,
        value,
    ...
    if (!isNil(value)) {
        if (enableMultiSelect) {
            ...
            selectedText = map(value, (selectedValue) => {
                ...
                const { selectedText: selectedTextItem } = getSelectedText(selectedValue, selectedLabelPath, getOptionById);
            ...
            ({ selectedText, selectedColor } = getSelectedText(value, selectedLabelPath, getOptionById));
...
function navHandler(event, dropDownRef, targetIndex, customOption, selectValue, items, enableMultiSelect, dropDownListRef, setTargetIndex) {
    if (event.which === 27) {
        // Escape
        // Close the tray.
...
function renderFiltering(isFilteringMessage) {
    return (React.createElement(SelectText, null,
        React.createElement(Spinner, { size: "xs" }),
        isFilteringMessage));
}
function renderOptions(finalId, listBoxId, labelledBy, customOption, items, filterTarget, noOptionsMessage, emptyFilterMessage, targetIndex, optionSeparator, size, enableMultiSelect, handleSelect, dropDownListRef, selectedLookup, isFiltering, isFilteringMessage) {
    let listItems = null;
    ...
    if (isFiltering) {
        ...
        listItems = React.createElement(SelectText, null, message);
    }
    else {
        ...
        if (!isNil(customOption)) {
            ...
            customValueElement = (React.createElement(SelectOption, { id: customId, targeted: customIsTarget, borderBottom: optionSeparator && items.length > 0, onClick: handleSelect, optionValue: null }, customOption));
        }
        listItems = (React.createElement(React.Fragment, null,
            customValueElement,
            map(items, (item, idx) => {
                ...
                return (React.createElement(SelectOption, { key: itemId, color: color, id: optionId, "aria-label": data['aria-label'], selected: selected, targeted: optionIsTarget, borderBottom: optionSeparator && idx + 1 < items.length, onClick: handleSelect, optionValue: itemId }, data.label));
            })));
    }
    return {
        options: (React.createElement(DropDownList, { ref: dropDownListRef, id: listBoxId, size: size, "aria-labelledby": labelledBy, "aria-multiselectable": enableMultiSelect }, listItems)),
        targetedElementId,
    };
}
...
const Select = forwardRef((props, ref) => {
    const { className, label, 'aria-label': ariaLabel, messages, failure, hidden, title, disabled, dockRight, minDrawerWidth, placeholder, Anchor, enableMultiSelect, anchorProps, onFocus, onBlur, id, 'aria-labelledby': ariaLabelledby, 'aria-describedby': ariaDescribedby, size, enableFiltering, value, onSelect, enableCustomValues, selectedLabelPath, options, onFilter, minFilterChar, onOpen, onClose, noOptionsMessage, optionSeparator, emptyFilterMessage, enableSorting, excludeValues, optionValuePath, optionLabelPath, isFiltering, isFilteringMessage, } = props;
    ...
    const contextOptions = useMemoDeps(buildContextOptions, [enableFiltering, enableSorting, excludeValues, filterTarget, optionValuePath, optionLabelPath]);
    ...
    const { filterMatchesValue, selectedText, selectedColor, selectedLookup, } = useMemoDeps(buildValueMeta, [value, filterTarget, selectedLabelPath, getOptionById, enableMultiSelect, customValueSetting]);
    // build meta data about the options
    const { filteredOptions, customOption } = useMemoDeps(buildFinalOptions, [items, filterTarget, customValueSetting, filterMatchesValue]);
    ...
    const dropdownNavHandler = useCallback((event) => navHandler(event, dropDownRef, targetIndex, customOption, selectValue, filteredOptions, enableMultiSelect, dropDownListRef, setTargetIndex), [targetIndex, customOption, selectValue, filteredOptions, enableMultiSelect]);
    ...
    const handleOpen = useCallback(() => {
        ...
        const newTargetIndex = findIndex(filteredOptions, (item) => selectedLookup[item.getId()]);
    ...
    return (React.createElement(InputFieldLabel, { className: classnames('ra-drop-down-input', className), id: id, label: label, messages: messages, failure: failure, "aria-labelledby": ariaLabelledby, "aria-describedby": ariaDescribedby, hidden: hidden, disabled: disabled, size: size }, ({ describedBy, labelledBy, inputId, finalId, }) => {
        const listBoxId = buildId(finalId, 'options-list');
        const { options: renderedOptions, targetedElementId } = renderOptions(finalId, listBoxId, labelledBy, customOption, filteredOptions, filterTarget, noOptionsMessage, emptyFilterMessage, targetIndex, optionSeparator, size, enableMultiSelect, selectValue, dropDownListRef, selectedLookup, isFiltering, isFilteringMessage);
        return (React.createElement(DropDown, { ref: dropDownRef, id: `${finalId}-drop-down`, className: classnames('ra-select'), trayClassName: classnames('ra-select-tray', `size-${size}`), disabled: disabled, minDrawerWidth: minDrawerWidth, onFocus: onFocus, onBlur: onBlur, maxTrayHeight: 216, onKeyDown: handleDropDownKeyDown, onOpen: handleOpen, onClose: handleClose, closeTrayOnClick: !enableMultiSelect, Anchor: Anchor, anchorProps: buildAnchorProps({
                id: inputId,
                color: selectedColor,
                value: selectedText,
                title,
            ...
            enableFiltering ? (React.createElement("input", { ref: filterInputRef, tabIndex: "-1", className: "ra-dd-list-filter", value: filterValue, onChange: handleFilterChange, onKeyDown: dropdownNavHandler, onClick: preventDefault, size: "1", "aria-label": "List Filter", "aria-activedescendant": targetedElementId })) : null,
            renderedOptions));
    }));
});
...
Anchor, ...exportPropTypes
// eslint-disable-next-line react/forbid-foreign-prop-types
 } = Select.propTypes;
const exportDefaultProps = Select.defaultProps;

export { Select as default, exportDefaultProps as defaultProps, exportPropTypes as propTypes };
