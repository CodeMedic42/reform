...
import { keys, forEach, reduce, isNil, isString, isArray, isEmpty, toPath, isFinite, isBoolean, startsWith, endsWith, toLower } from 'lodash-es';
...
doesMatch = function doesMatchMethod(item, values, targetPath, matchMethod) {
    let target = item;
    for (let counter = 0; counter < targetPath.length && !isNil(target); counter += 1) {
        const targetPathPart = targetPath[counter];
        if (targetPathPart === '*') {
            return arrayDoesMatch(target, values, targetPath.slice(counter + 1), matchMethod);
        }
...
function processStringValue(rawValues) {
    return reduce(rawValues, (acc, rawValue) => {
...
function processRawValues(rules, values) {
    ...
    if (!isNil(values)) {
        ...
        forEach(cleanedValues, (value, key) => {
...
function containsMatch(values, target) {
    ...
    const finalIndex = reduce(values, (acc, valueItem) => {
...
function processRule(rawRule) {
    ...
    const paths = reduce(rule.paths, (acc, targetPath) => {
...
function processRules(rules) {
    ...
    return reduce(cleanedRules, (acc, rule, key) => {
...
function useFilterSettings(settings) {
    ...
    const { enabled, valueItemsList: processedValues } = useMemo(() => processRawValues(processedRules, values), [processedRules, values]);
    const processValue = useCallback((item) => {
        ...
        forEach(processedValues, (processedValue, key) => {
            ...
            let resultCount = reduce(rule.paths, (acc, path) => {
                let ret = doesMatch(item, processedValue, path, rule.matchMethod);
                ret /= pathCount;
                return acc + ret;
            }, 0);
            resultCount /= count;
...
export { useFilterSettings as default };
