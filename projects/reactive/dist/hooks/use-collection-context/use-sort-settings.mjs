...
import { orderBy, isArray, forEach, isNil, isString, isEmpty } from 'lodash-es';
...
function processSortRules(sortRules) {
    ...
    forEach(preRules, (sortRule) => {
        ...
        if (isString(sortRule)) {
            ...
            else if (isNil(sortRule.type) || sortRule.type === 'property') {
...
function useSortSettings(settings) {
    ...
    const { by, order, filterPriorityUsed, } = useMemo(() => processSortRules(rules), [rules]);
...
export { useSortSettings as default };
